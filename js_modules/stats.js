...
stats_button.addEventListener('click', () => {
    ...
    difficult_words.onclick = () => {
        ...
        functionCreateCards(difficultWordsArray);
    }
})
...
function generateStatsRow() {
    for (let category in wordsForStats) {
        for (word of wordsForStats[category]) {
            ...
            stats_percent = (100 * Number(statsStorage.getItem(`${word.name}.correct`))) / (Number(statsStorage.getItem(`${word.name}.correct`)) + Number(statsStorage.getItem(`${word.name}.incorrect`)));
...
function sortTable(arr_rows, th_number) {
